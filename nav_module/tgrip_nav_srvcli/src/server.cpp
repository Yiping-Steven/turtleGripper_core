#include <tgrip_nav_srvcli/server.hpp>

tgrip_nav_server::tgrip_nav_server (ros::NodeHandle& nh): nh( nh ){

  as.reset( new ActionServer(nh, "action_server", false) );
  as->registerGoalCallback( boost::bind( &tgrip_nav_server::goalCB, this) );
  // as->registerGoalCallback( boost::bind( &tgrip_nav_server::preemptCB, this) );
  as->start();

  subOdom = nh.subscribe("/odom", 1000, & tgrip_nav_server::odomCallback, this);
  pubCmd = nh.advertise<geometry_msgs::Twist>("/cmd_vel_mux/input/teleop", 10);

  angSpeedTol = PI/10; // unit in rad/seconds
  posSpeedTol = 1/10; // unit in meter/seconds

}

void tgrip_nav_server::goalCB(){
  std::cout << "tgrip_nav_server::goalCB" << std::endl;
...
void tgrip_nav_server::odomCallback(const nav_msgs::Odometry::ConstPtr& odomMsgPtr){
